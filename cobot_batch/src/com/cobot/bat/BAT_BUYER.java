	...
	public static void main(String[] args) {
		...
		while(true){
			...
			for(int i = 0 ; i < RCM_LIST.size();i++){
				...
				if(MY_COINS.containsKey(ccd)){
					...
					try {
						...
						l("�̹� ������ �����Դϴ�. " + ccd + ", PER_CH : " + PER_CH + ", BUY_PRICE : " + COIN_INFO.get("BUY_PRICE") + ", CUR_PRICE : " + RCM_Map.get("CUR_PRICE") );
					} catch (Exception e) {
					...
					if( MY_COINS.size() < LIMIT_NUMBER_COIN ){
						...
						l("[" + ccd + "] : ������ [" + RCM_Map.get("CUR_PRICE") + "]�� ���ſ�û�Ͽ����ϴ�.");
			...
			for(java.util.Iterator it = MY_COINS.keySet().iterator(); it.hasNext();){
				...
				l( ccd + " : ������ : " + VAL_MAP.get("PER_CH")  + ", BUY_PRICE : " + VAL_MAP.get("BUY_PRICE") + ", BUY_DTTM : " + VAL_MAP.get("BUY_DTTM"));
			}
